...
header("Content-type:text/html;charset=utf-8");
    ...
    $real_usr_hour = $temp_array[5];
    $real_usr_minute = $temp_array[6];
    $real_usr_second = $temp_array[7];
    $phone_id = $temp_array[8];
    $temp_array1 = explode('-', $usr_time);
    $usr_time_new = $temp_array1[0] . "-" . $usr_hour . ":" . $usr_minute . ":" . '0';
    $usr_time_real = $temp_array1[0] . "-" . $real_usr_hour . ":" . $real_usr_minute . ":" . $real_usr_second;
    ...
    $file_path = $file_path . basename($imgname);
		...
		$sql = "insert into photo_path_new (`user_name`,`phone_id`,`user_time`,`upload_time`,`photo_path`, `photo_name`, `photo_type`)values('$phone_id','$usr_name','$usr_time_new','$usr_time_real','$file_path','$imgname','food')";
   ...
   $current_time = date('Y-m-d h:i:s', time());
		...
		if($file_num >= 1){
			...
			for($x = 0; $x < count($new_result_array); $x++){
				...
				//echo $temp_imagename;
				//echo $temp_imagefood;
				$sql = "UPDATE photo_path_new SET photo_type = '$temp_imagefood' WHERE photo_name = '$temp_imagename'";
		    $stmt = mysqli_prepare($conn,$sql);
  			mysqli_stmt_execute($stmt);
			}
		    ...
		    $stmt1 = mysqli_prepare($conn,$sql1);
  			...
  			$sql2 = "UPDATE photo_path_new SET photo_kind = '$temp_imagekind' WHERE photo_name = '$temp_imagename'";
			...
			for($x = 0; $x < count($temp_array3); $x++){
				...
				$sql2 = "UPDATE photo_path_new SET workday = '$temp_workday', weekend = '$temp_weekend' WHERE photo_name = '$temp_imagename'";
		    $stmt2 = mysqli_prepare($conn,$sql2);
  			mysqli_stmt_execute($stmt2);
			}
		...
		if($file_num >= 5){
			...
			for($x = 0; $x < count($temp_array2); $x++){
				...
				//echo $temp_imagecal;
				//echo $temp_imagekind;
				$sql1 = "UPDATE photo_path_new SET photo_cal = '$temp_imagecal' WHERE photo_name = '$temp_imagename'";
		    $stmt1 = mysqli_prepare($conn,$sql1);
  			mysqli_stmt_execute($stmt1);
