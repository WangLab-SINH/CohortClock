...
files = "/var/www/html/deeplearning_photo/temp_photo_calorie/" + sys.argv[1] + "/"
...
data_dir = '/var/www/html/deeplearning_photo/temp_photo_calorie/'
...
def predict():
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(val_dataloaders):
            ...
            values, indices = outputs.topk(5, dim=1, largest=True, sorted=True)
            # print("batch %d" % i)
