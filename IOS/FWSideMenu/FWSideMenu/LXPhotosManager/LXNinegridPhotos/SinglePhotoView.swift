...
public enum SinglePhotoViewTapType {
    ...
    case takePhotoView(SinglePhotoView)
}
...
public class SinglePhotoView: UIView {
    ...
    //private var takePhotoView: UIImageView!
    
    //public var buttonView: UIImageView!
...
extension SinglePhotoView {
    ...
    public override func layoutSubviews() {
       ...
       imgView.frame = CGRect(x: 0, y: 0, width: self.frame.width, height: self.frame.height)
        //buttonView.frame = CGRect(x: 150, y: 400, width: self.frame.width, height: self.frame.height)
       //设置closeButton尺寸
        ...
        if case let .add(isAdd: isAdd, config: config) = self.type {
            if isAdd {
                if let addImg = config.addImage {
                   ...
                   // buttonView.image = config.buttonImage
                }else{
                    imgView.image = UIImage.named("NinePhotoAdd")
                    //buttonView.image = UIImage.named("NinePhotoAdd")
                }
            }else{
...
//        buttonView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(takePhotoView(_:))))
//        
    ...
    ///点击图片
    @objc private func takePhotoView(_ gesture: UIGestureRecognizer) {
        // 预防连续点击
        gesture.view?.isUserInteractionEnabled = false
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1.0) {
             gesture.view?.isUserInteractionEnabled = true
